...
The contract covers GPU, CPU, RAM, VRAM and storage, but each still requires its subsystem-specific semantic benchmark. For RAM/VRAM/storage, capacity alone is insufficient; access, bandwidth, latency and persistence/durability requirements must be part of the benchmark.
...
## Next research gate
Build concrete equivalence-contract fixtures for compute and memory/storage, then populate the contract from measured competitive baselines and real cloud-QPU execution. Until those fields are measured, INV-025/026 remain NOT_YET_DEMONSTRATED.
