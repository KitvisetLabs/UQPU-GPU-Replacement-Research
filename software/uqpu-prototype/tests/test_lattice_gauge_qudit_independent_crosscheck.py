...
def test_full_space_projection_has_expected_dimension():
    assert len(ZERO_MAGNETIZATION_FULL_INDICES) == 18
    assert len(set(ZERO_MAGNETIZATION_FULL_INDICES)) == 18


def test_independent_tensor_product_builder_matches_batch042_state_builder():
    for inverse_g_squared in COUPLINGS:
        ...
        assert _max_abs_matrix_difference(independent, batch042) < 1e-12


def test_independent_jacobi_ground_states_have_small_residuals():
    for inverse_g_squared in COUPLINGS:
        result = independent_ground_state(inverse_g_squared)
        assert result.residual_norm < 2e-12
        assert result.jacobi_rotations < 1_000


def test_batch042_power_iteration_energy_and_observable_are_crosschecked():
    ...
    assert maximum_energy_difference < 1e-12
    assert maximum_plaquette_difference < 1e-7
    ...
    assert 1e-6 < maximum_batch042_residual < 1e-5


def test_independent_full_spectrum_is_sorted_and_ground_observable_is_consistent():
    hamiltonian = projected_hamiltonian(1.0)
    eigenvalues, eigenvectors, _ = symmetric_jacobi_eigendecomposition(hamiltonian)
    assert len(eigenvalues) == 18
    assert eigenvalues == sorted(eigenvalues)

    ground = eigenvectors[0]
    assert math.isclose(sum(value * value for value in ground), 1.0, abs_tol=1e-12)
    assert eigenpair_residual(hamiltonian, eigenvalues[0], ground) < 2e-12
    assert math.isclose(
...
def test_evidence_classification_keeps_independence_boundary_and_nonclaims():
    evidence = classification()
    assert evidence["classification"] == "LATTICE_QED_INDEPENDENT_OPERATOR_CROSSCHECK"
    assert evidence["evidence_level"] == "INTERNAL_INDEPENDENT_IMPLEMENTATION_CROSSCHECK"
    assert evidence["full_hilbert_dimension"] == 48
    assert evidence["projected_sector_dimension"] == 18
    assert "not an independent external team" in evidence["independence_boundary"]
