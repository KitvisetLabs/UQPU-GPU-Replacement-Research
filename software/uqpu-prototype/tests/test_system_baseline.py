...
class SystemBaselineTests(unittest.TestCase):
    def test_complete_stack_total_includes_npu(self):
        self.assertEqual(ConventionalSystemCost(cpu=1,gpu=2,npu=6,accelerator_memory=3,host_memory=4,storage=5).total,21)
    def test_advantage(self):
        r=compare_system_cost(ConventionalSystemCost(gpu=100,npu=20,host_memory=10),UQCSSystemCost(quantum_compute=.5,memory=.5))
        self.assertEqual(r.ratio,130); self.assertTrue(r.reaches_100x); self.assertFalse(r.reaches_100m_x)
